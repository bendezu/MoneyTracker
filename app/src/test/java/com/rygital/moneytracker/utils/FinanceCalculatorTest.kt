...
class FinanceCalculatorTest {
    ...
    val transactions = listOf(
            DetailedTransaction(0, EXPENSE, BigDecimal(1), eur.label, eur.symbol, eur.rateToUsd,
                    1, 0, 0, 0,0,0, Date()),
            DetailedTransaction(0, INCOME, BigDecimal(2), eur.label, eur.symbol, eur.rateToUsd,
                    3, 0, 0, 0,0,0, Date()),
            DetailedTransaction(0, EXPENSE, BigDecimal(3), eur.label, eur.symbol, eur.rateToUsd,
                    2, 0, 0, 1,0,0, Date()),
            DetailedTransaction(0, EXPENSE, BigDecimal(1), eur.label, eur.symbol, eur.rateToUsd,
                    1, 0, 0, 1,0,0, Date())
    )
    ...
    @Test
    fun `Should return empty accountsData when no transactions passed`() {
        val accountsData = getAccountsData(Collections.emptyList(), usd, eur)
        assertEquals(0, accountsData.size)
    }

    @Test
    fun `balances should be same for the same currencies`() {
        val accountsData = getAccountsData(transactions, usd, usd)
    ...
    fun `Basic accountsData calculation test`() {
        val accountsData = getAccountsData(transactions, eur, usd)
    ...
    fun `Should return empty chartData when no transactions passed`() {
        val chartData = getChartData(Collections.emptyList(), usd)
        assertEquals(0, chartData.size)
    }

    @Test
    fun `Basic chartData calculation test`() {
        val chartData = getChartData(transactions, eur)
